...
class Keyword extends DBRecord {
	...
	static public function search(  $keyword = "", 
		...
		if( $keyword != "" ) {
			if( $use_regexp ) {
				$options .= " AND CONCAT(title,description) REGEXP '".mysqli_real_escape_string($dbh, $keyword)."'";
			}
			else {
				// 全角半角
				// 場当たり的に対応
				...
				$options .= " AND ( CONCAT(title,' ',description) like '%".mysqli_real_escape_string($dbh, $keyword)."%'";
				
				if( $f_zennum ) {
					$options .= " OR CONCAT(title,' ',description) like '%".mysqli_real_escape_string($dbh, mb_convert_kana( $keyword, 'KVN',"UTF-8" ))."%'";
				}
				if( $f_zenal ) {
					$options .= " OR CONCAT(title,' ',description) like '%".mysqli_real_escape_string($dbh, mb_convert_kana( $keyword, 'KVR', "UTF-8" ))."%'";
				}
				if( $f_zenal && $f_zennum ) {
					$options .= " OR CONCAT(title,' ',description) like '%".mysqli_real_escape_string($dbh, mb_convert_kana( $keyword, 'KVRN', "UTF-8" ))."%'";
				}
				$options .= ") ";
			}
		}
