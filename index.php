...
foreach($channels as $ch) {
    if ($ch["type"] == $type) {
        $channel_map[$ch["channel_disc"]] = $ch["channel"];
    }
}
...
$types = array();
$i = 0;
foreach($types_ch as $type_ch) {
	$types[$i]['selected'] = $type == $type_ch ? 'class="selected"' : "";
	$types[$i]['link'] = $_SERVER['SCRIPT_NAME'] . "?type=$type_ch&length=".$program_length."&time=".date( "YmdH", $top_time);
	$types[$i]['name'] = $type_ch;
	$i++;
}
