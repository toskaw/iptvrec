...
class DBRecord {
    ...
    function __construct( $table, $property = null, $value = null ) {
		...
		if( ($property == null) || ($value == null) ) {
			...
			if( $this->__record_data === NULL ) throw new exception( 'construct:'.$this->__table.'に'.$property.'='.$value.'はありません' );
			// 最初にヒットした行のidを使用する
	...
	function __set( $property, $value ) {
		...
		if( array_key_exists( $property, $this->__record_data ) ) {
            if ($this->__record_data[$property] !== $value ) {
...
//					$sqlstr .= ' '.$property." = '".mysqli_real_escape_string(stripslashes($value))."',";		// ここだけ直す場合
					$sqlstr .= ' '.$property." = '".mysqli_real_escape_string(self::$__dbh, $value)."',";
				}
				$sqlstr = rtrim($sqlstr, ',' );
