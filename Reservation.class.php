...
class Reservation {
	...
	public static function custom(
		...
		if( $start_time < (time() + $settings->former_time + 60) ) {	// 現在時刻より小さい＝すでに開始されている番組
			$start_time = time() + 70;		// 録画開始時間を70秒先に設定する
			$at_start = $start_time;
		        $rec_start = $start_time - $settings->former_time;
		        $sleep_time = $rec_start - $at_start;
		}
		else {
			$at_start = $start_time - 60;
		        $rec_start = $start_time - $settings->former_time;
		        $sleep_time = $rec_start - $at_start;
		}
...
*/
			...
			if( is_resource( $process ) ) {
				if ($check_epg == 0) {
                                        fwrite($pipes[0], "set -e\n" );
                                        fwrite($pipes[0], "trap '".COMPLETE_CMD." ".$rrec->id." > /dev/null 2>&1' EXIT \n" );
					fwrite($pipes[0], DO_RECORD . "> /dev/null 2>&1\n" );
					if( $settings->use_thumbs == 1 ) {
						fwrite($pipes[0], $gen_thumbnail." > /dev/null 2>&1\n" );
					}
				}
				else {
					fwrite($pipes[0], INSTALL_PATH."/checkepg.php ".$rrec->id." > /dev/null 2>&1\n" );
				}
				fclose($pipes[0]);
				// 標準エラーを取る
			...
			// エラー
			reclog( "Reservation::custom job番号の取得に失敗".$rstring,E_ERROR );
			$rrec->delete();
	...
	public static function cancel( $reserve_id = 0, $program_id = 0 ) {
		...
		try {
			...
			if( ! $rec->complete ) {
                ...
                $log = "";
				$log = exec( $settings->atrm . " " . $rec->job );
				reclog("Reservation::cancel ジョブ番号".$rec->job."を削除:".$log);
			}
			$rec->delete();
		}
		catch( Exception $e ) {
